...
public class GamePageTest extends E2EBase {
        ...
        @Test(priority = 10, description = "Verify a DRM-free label or protection indicator is present (GOG's core brand promise)")
        public void testDrmFreeIndicator() {
                TestUtils.pause(800);
                boolean present = TestUtils.isElementPresent(driver,
                                By.xpath("//*[contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ',"
                                                + "'abcdefghijklmnopqrstuvwxyz'),'drm')]"
                                                + "[not(ancestor::footer)][not(self::script)]"))
                                || TestUtils.isElementPresent(driver,
                                                By.cssSelector("[class*='drm'], [class*='nodrm'], [class*='protection']"));
                Assert.assertTrue(present,
                                "A DRM-free label or protection indicator must be visible on the GOG game page"
                                                + " - this is GOG's core promise of DRM-free content");
        }

        @Test(priority = 11, description = "Verify a user rating or community review section is present on the game page")
        public void testRatingOrReviewsPresent() {
                TestUtils.pause(800);
                boolean present = TestUtils.isElementPresent(driver,
                                By.cssSelector("[class*='rating'], [class*='review'], [class*='score'], [class*='star']"))
                                || TestUtils.isElementPresent(driver,
                                                By.xpath("//*[contains(@class,'rating') or contains(@class,'review')]"
                                                                + "[string-length(normalize-space(.)) > 0][not(self::script)]"));
                Assert.assertTrue(present,
                                "A user rating or community review section must be visible on The Witcher 3 product page");
        }
}
